...
const CountBox = ({ selected, resolvedData }) => {
  ...
  // const inProgressData = data.filter((element) => element.status == "Open");
  // const ResolvedData = tickets.filter((element) => element.status == "In-Progress");

  return (
    <Container>
      <div className="flex justify-between items-center gap-6 py-20">
        <div className="flex-1 rounded-lg p-10 text-white h-[250px] bg-linear-to-r from-[#632EE3] to-[#9F62F2] flex flex-col justify-center items-center relative">
        ...
        <div className=" flex-1 rounded-lg p-10 text-white h-[250px] bg-linear-to-r from-[#54CF68] to-[#00827A] flex flex-col justify-center items-center relative">
          <img className="absolute left-0 top-0" src={bg1} alt="" />
          <img className="absolute right-0 top-0" src={bg2} alt="" />
